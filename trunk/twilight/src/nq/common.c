...
/*
void
Q_memset (void *dest, int fill, int count)
{
	int         i;
...
void
Q_memcpy (void *dest, void *src, int count)
{
	int         i;
...
int
Q_memcmp (void *m1, void *m2, int count)
{
	while (count) {
		count--;
...
void
Q_strncpy (char *dest, char *src, int count)
{
	while (*src && count--) {
		*dest++ = *src++;
	}
...
int
Q_strncmp (char *s1, char *s2, int count)
{
	while (1) {
		if (!count--)
			return 0;
...
*/
int
COM_FindFile (char *filename, int *handle, FILE ** file)
{
	searchpath_t *search;
...
*/
int
COM_OpenFile (char *filename, int *handle)
{
