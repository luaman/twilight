		...
		//==================
			...
			case OP_STOREP_FNC:		// pointers
				...
				if (OPB->_int % pr_edict_size < ((Uint8 *)&sv.edicts->v - (Uint8 *)sv.edicts))
				{
					pr_xstatement = st - pr_statements;
...
#endif
				ptr = (eval_t *)((Uint8 *)sv.edicts + OPB->_int);
				ptr->_int = OPA->_int;
				break;
			case OP_STOREP_V:
#if PRBOUNDSCHECK
			...
			case OP_ADDRESS:
				...
				OPC->_int = (Uint8 *)((int *)&ed->v + OPB->_int) - (Uint8 *)sv.edicts;
				break;
		...
		//==================
			...
			case OP_RETURN:
				...
				if (pr_depth <= exitdepth)
					return;		// all done
				if ((int) pr_trace != cachedpr_trace)
