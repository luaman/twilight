		...
		vec3_t outmins, vec3_t outmaxs)
{
	...
	if (size[0] < 3)
		hull = &cmodel->extra.brush->hulls[0]; // 0x0x0
	else if (size[0] <= 32)
		hull = &cmodel->extra.brush->hulls[1]; // 32x32x56
	else
		hull = &cmodel->extra.brush->hulls[2]; // 64x64x88
		...
		vec3_t normal)
{
	...
	TraceLine_Raw (mdl->extra.brush->hulls, start, end, &trace);
