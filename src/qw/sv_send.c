...
SV_Multicast (vec3_t origin, int to)
{
	...
	if (!leaf)
		...
		leafnum = leaf - sv.worldmodel->extra.brush->leafs;
	...
	switch (to) {
		...
		case MULTICAST_PHS_R:
			reliable = true;			// intentional fallthrough
		case MULTICAST_PHS:
			mask = sv.phs + leafnum * 4 * ((sv.worldmodel->extra.brush->numleafs + 31) >> 5);
			break;

		case MULTICAST_PVS_R:
			reliable = true;			// intentional fallthrough
		case MULTICAST_PVS:
			mask = sv.pvs + leafnum * 4 * ((sv.worldmodel->extra.brush->numleafs + 31) >> 5);
			break;
	...
	for (j = 0, client = svs.clients; j < MAX_CLIENTS; j++, client++) {
		...
		if (leaf) {
			...
			leafnum = leaf - sv.worldmodel->extra.brush->leafs - 1;
